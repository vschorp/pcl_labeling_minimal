...
class PointLabelerRos {
 ...
 private:
  void initialize();
  void pclAndImgCallback(const sensor_msgs::ImageConstPtr& img_msg, const sensor_msgs::PointCloud2ConstPtr& lidar_pcl_msg);

  Eigen::Matrix3Xd preprocessPcl(pcl::PointCloud<pcl::PointXYZ>& lidarPclCloud);
  void removePtsOutsideOfRange(const pcl::PointCloud<pcl::PointXYZ>& cloud_in, pcl::PointCloud<pcl::PointXYZ>& cloud_out, float min_range, float max_range); 
  ...
  std::string rgb_pcl_topic_;

  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::PointCloud2> ApproxTimePolicy_;
